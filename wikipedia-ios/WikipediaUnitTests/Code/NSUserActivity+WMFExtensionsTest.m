...
- (void)testPlaces_givenCoordinates_shouldUpdateActivityWithCoordinates {
  // Arrange
  NSNumber *latitude = @52.3547498;
  NSNumber *longitude = @4.8339215;
  NSString *urlString = [NSString stringWithFormat:@"wikipedia://places/coordinates?lat=%@&long=%@", latitude, longitude];
  NSURL *url = [NSURL URLWithString:urlString];
  
  // Act
  NSUserActivity *activity = [NSUserActivity wmf_activityForWikipediaScheme:url];
  NSNumber *lat = activity.userInfo[@"WMFPlacesCoordinatesLatitude"];
  NSNumber *lon = activity.userInfo[@"WMFPlacesCoordinatesLongitude"];

  // Assert
  XCTAssert([lat isEqualToNumber:@52.3547498]);
  XCTAssert([lon isEqualToNumber:@4.8339215]);
}
